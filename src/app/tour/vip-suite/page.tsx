...
export default function VipSuitePage() {
  ...
  const features = t("tour.vip.features", { returnObjects: true }) as string[];
  const faq = t("tour.vip.faq", { returnObjects: true }) as Array<{
    q: string;
    a: string;
  }>;
        ...
        <div className="max-w-6xl mx-auto px-6 py-24 md:py-28 relative z-10 grid lg:grid-cols-2 gap-12 items-center">
          <div>
            <p className="text-white/90 font-semibold tracking-widest text-xs uppercase mb-5">
              {t("tour.vip.heroLabel")}
            </p>
            <h1 className="text-4xl md:text-6xl font-serif font-bold leading-tight mb-6">
              {t("tour.vip.title")}
              <span className="block text-cream">{t("tour.vip.subtitle")}</span>
            </h1>
            <p className="text-lg md:text-xl text-white/90 max-w-xl leading-relaxed">
              {t("tour.vip.description")}
            </p>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14">
            <p className="text-principal font-semibold tracking-widest text-xs uppercase mb-4">
              {t("tour.vip.highlightsLabel")}
            </p>
            <h2 className="text-3xl md:text-5xl font-serif font-bold text-gray-900">
              {t("tour.vip.highlightsTitle")}
            </h2>
            <p className="text-gray-600 text-lg max-w-2xl mx-auto mt-4">
              {t("tour.vip.highlightsSubtitle")}
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {highlights.map((item, index) => (
              ...
              >
                <div className="w-10 h-10 rounded-full bg-wine/10 text-wine flex items-center justify-center font-bold mb-4">
                  0{index + 1}
                </div>
        ...
        <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-8">
            <div>
              <p className="text-principal font-semibold tracking-widest text-xs uppercase mb-3">
                {t("tour.vip.detailsLabel")}
              </p>
              <h2 className="text-3xl md:text-4xl font-serif font-bold text-gray-900">
                {t("tour.vip.detailsTitle")}
              </h2>
            </div>

            <p className="text-gray-700 text-lg leading-relaxed">
              {t("tour.vip.detailsText")}
            </p>

            <div className="grid sm:grid-cols-2 gap-6">
              {features.map((feature) => (
                <div key={feature} className="flex items-start gap-3">
                  <div className="mt-2 w-2 h-2 bg-wine rounded-full"></div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Link
                href={getLocalizedPath("/book", currentLang)}
                className="bg-wine text-white px-8 py-4 rounded-lg font-bold text-sm tracking-wider hover:bg-wine/90 transition-all text-center"
              >
                {t("tour.vip.reserve")}
              </Link>
              <Link
                href={getLocalizedPath("/rooms", currentLang)}
                className="border-2 border-wine text-wine px-8 py-4 rounded-lg font-bold text-sm tracking-wider hover:bg-wine hover:text-white transition-all text-center"
              >
                {t("tour.vip.viewAll")}
              </Link>
            </div>
          ...
          <div className="grid gap-6">
            <div className="rounded-3xl overflow-hidden shadow-xl">
              <img
                src="/habitacion_vip1.jpg"
                alt="VIP suite lounge"
                className="w-full h-56 md:h-64 object-cover"
              />
            </div>
            <div className="rounded-3xl overflow-hidden shadow-xl">
              <img
                src="/habitacion_vip2.jpg"
                alt="VIP suite details"
                className="w-full h-56 md:h-64 object-cover"
              />
            </div>
          </div>
