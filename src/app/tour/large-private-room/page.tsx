...
export default function LargePrivateRoomPage() {
  const { t, i18n } = useTranslation();
  const currentLang = i18n.language?.startsWith("es") ? "es" : "en";
  const highlights = t("tour.large-private.highlights", {
    returnObjects: true,
  }) as Array<{ title: string; desc: string }>;
  const features = t("tour.large-private.features", {
    returnObjects: true,
  }) as string[];
  const faq = t("tour.large-private.faq", { returnObjects: true }) as Array<{
    q: string;
    a: string;
  }>;
        ...
        <div className="max-w-6xl mx-auto px-6 py-24 md:py-28 relative z-10 grid lg:grid-cols-2 gap-12 items-center">
          <div>
            <p className="text-white/90 font-semibold tracking-widest text-xs uppercase mb-5">
              {t("tour.large-private.heroLabel")}
            </p>
            <h1 className="text-4xl md:text-6xl font-serif font-bold leading-tight mb-6">
              {t("tour.large-private.title")}
              <span className="block text-cream">
                {t("tour.large-private.subtitle")}
              </span>
            </h1>
            <p className="text-lg md:text-xl text-white/90 max-w-xl leading-relaxed">
              {t("tour.large-private.description")}
            </p>
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-4">
              ...
              >
                {t("tour.large-private.book")}
              </Link>
              <Link
                href={getLocalizedPath("/tour/360/large-private", currentLang)}
          ...
          <div className="relative">
            ...
            <div className="relative rounded-3xl overflow-hidden shadow-2xl border border-white/20">
              <img
                src="/Amplia habitacion con sofa cama y vista para paciente y acompanate con todas las amenidades incluidas en Tijuana.webp"
                alt="Large private recovery room"
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14">
            <p className="text-principal font-semibold tracking-widest text-xs uppercase mb-4">
              {t("tour.large-private.highlightsLabel")}
            </p>
            <h2 className="text-3xl md:text-5xl font-serif font-bold text-gray-900">
              {t("tour.large-private.highlightsTitle")}
            </h2>
            <p className="text-gray-600 text-lg max-w-2xl mx-auto mt-4">
              {t("tour.large-private.highlightsSubtitle")}
            </p>
          </div>
        ...
        <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-8">
            <div>
              <p className="text-principal font-semibold tracking-widest text-xs uppercase mb-3">
                {t("tour.large-private.detailsLabel")}
              </p>
              <h2 className="text-3xl md:text-4xl font-serif font-bold text-gray-900">
                {t("tour.large-private.detailsTitle")}
              </h2>
            </div>

            <p className="text-gray-700 text-lg leading-relaxed">
              {t("tour.large-private.detailsText")}
            </p>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Link
                href={getLocalizedPath("/book", currentLang)}
                className="bg-wine text-white px-8 py-4 rounded-lg font-bold text-sm tracking-wider transition-all text-center"
              >
                {t("tour.large-private.reserve")}
              </Link>
              <Link
          ...
          <div className="grid gap-6">
            <div className="rounded-3xl overflow-hidden shadow-xl">
              <img
                src="/Amplia habitacion con sofa cama y vista para paciente y acompanate con todas las amenidades incluidas en Tijuana.webp"
                alt="Large private room ambiance"
      ...
      <section className="py-16 md:py-20 px-6 bg-gradient-to-b from-cream/60 to-white">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-5xl font-serif font-bold text-gray-900">
              {t("tour.large-private.faqTitle")}
            </h2>
            <p className="text-gray-600 text-lg mt-4">
              {t("tour.large-private.faqSubtitle")}
            </p>
          </div>
      ...
      <section className="py-16 md:py-20 px-6 bg-wine text-white">
        <div className="max-w-5xl mx-auto text-center">
          <h2 className="text-3xl md:text-5xl font-serif font-bold mb-4">
            {t("tour.large-private.ctaTitle")}
          </h2>
          <p className="text-lg max-w-2xl mx-auto mb-8">
            {t("tour.large-private.ctaSubtitle")}
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link
              ...
              className="bg-white text-wine px-8 py-4 rounded-lg font-bold text-sm tracking-wider hover:bg-cream transition-all shadow-lg text-center"
            >
              {t("tour.large-private.ctaBook")}
            </Link>
            <Link
              href={getLocalizedPath("/contact", currentLang)}
              className="border-2 border-white px-8 py-4 rounded-lg font-bold text-sm tracking-wider hover:bg-white hover:text-wine transition-all text-center"
            >
              {t("tour.large-private.ctaTalk")}
            </Link>
          </div>
        </div>
      </section>
