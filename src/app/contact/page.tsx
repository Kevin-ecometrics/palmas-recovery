...
const handleSubmitForm = async (
  ...
  t: (key: string) => string,
  formRef: React.RefObject<HTMLFormElement | null>, // Añadir formRef como parámetro
) => {
          ...
          <div className="bg-white rounded-xl shadow-lg p-8 w-full lg:w-2/3">
            ...
            >
              ...
              <button
                type="submit"
                disabled={status === "sending"}
                className="w-full bg-principal text-white font-medium py-3 rounded-md hover:bg-principal-dark transition flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
              >
                {status === "sending" ? (
                  <>
                    <svg
