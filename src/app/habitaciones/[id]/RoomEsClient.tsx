...
import RoomContent from "@/app/rooms/[id]/RoomContent";
...
import { getRoomIdFromPath, getRoomPath } from "@/i18n/slugRoutes";

type Props = {
  id: string;
};

export default function RoomEsClient({ id }: Props) {
  const router = useRouter();
  const { t, i18n } = useTranslation();
  const currentLang = i18n.language?.startsWith("es") ? "es" : "en";
  const roomId = getRoomIdFromPath(id);
